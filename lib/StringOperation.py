...
class StringOperation():
    def __init__(self, load_flag="load"):
        ...
        self.mw2v = MyWord2Vec()
        fname = const.word2vec_train_file
        if load_flag == "train":
            self.mw2v.train(fname)
        self.word2vec_model = self.mw2v.load_model()
    ...
    def sentens_array_to_vec(self, sentens_arr):
        ...
        for value in sentens_arr:
            __vec = self.mw2v.str_to_vector(self.word2vec_model, value)
    ...
    def sentens_vec_to_sentens_arr(self, sentens_vec):
        ...
        for value in sentens_vec:
            __word = self.mw2v.vec_to_word(self.word2vec_model, value)
            __arr.append(__word)
        return __arr
    ...
    def sentens_vec_to_sentens_arr_prob(self, sentens_vec):
        __arr = []
        for value in sentens_vec:
            __prob_word = self.mw2v.vec_to_some_word(
                self.word2vec_model, value, 5)
            # print(__prob_word)
            __word_list = []
            __prob = []
